...
class Map {
 ...
 public:
  ...
  Map() = delete;  // non ha senso una mappa vuota
  Map(std::string_view);

  constexpr bool isOutOfBoundary(std::size_t x, std::size_t y) const;
  void spriteOverlay();
  ...
  constexpr const std::array<sf::Sprite, tilePaths.size()> &getTileSprites()
      const;
  void addObject(Position, Type);
  void resetObject(const Position);

  // Quale oggetto c'è in quella posizione?
  constexpr Objects &At(std::size_t x, std::size_t y);
  constexpr const Objects &At(std::size_t x, std::size_t y) const;
  constexpr Objects &At(Position);

  const std::vector<Position> getPositions(Type) const;
  void pathFinder(Position, Direction, const std::array<Direction, 4> &, bool);
};

constexpr bool Map::isOutOfBoundary(std::size_t x, std::size_t y) const {
  return ((x > MapSize::width - 1) || (y > MapSize::height - 1));
}

constexpr const std::array<sf::Sprite, tilePaths.size()> &Map::getTileSprites()
    const {
  return tileSprites;
}

constexpr Objects &Map::At(std::size_t x, std::size_t y) {
  return m_objects[y][x];
}
constexpr const Objects &Map::At(std::size_t x, std::size_t y) const {
  return m_objects[y][x];
}
constexpr Objects &Map::At(Position position) {
